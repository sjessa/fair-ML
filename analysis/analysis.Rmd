...
comp_tidy$method = factor(comp_tidy$method, levels = c("None", "Massaging", "Reweighting", "Uniform sampling", "2 Naive Bayes"))
...
comp_tidy %>% gather(measure, value, impact_ratio, odds_ratio, elift_ratio) %>%
  ...
  xlab(NULL) + ylab("Discrimination") +
  facet_grid(measure ~ dataset, switch = "y") +
  theme_bw() +
...
all_tidy$method = factor(all_tidy$method, levels = c("None", "Massaging", "Reweighting", "Uniform sampling", "2 Naive Bayes"))
...
all_tidy %>% 
  ...
  ylim(0, 1) +
  xlab("Discrimination") +
  #geom_text_repel(aes(x = value, y = accuracy, label = model)) +
  facet_grid(measure ~ dataset, switch = "y") +
  theme_bw() +
...
thresh %>% gather(statistic, value, elift, impact, odds, acc) %>%
  ...
  facet_grid(statistic ~ dataset, switch = "y") +
  theme_bw() +
  theme(legend.position = "bottom")
